...
class FrameSwitchTestCase(unittest.TestCase):
    ...
    def test(self):    
        ...
        # error when next line uncommented
        text = " ".join(['<a href="http://www.report.com">%s</a>' % w for w in text.split()])
        text += " "
        
        story = []
        story.append(Paragraph(text * 40, style=normal))

        doc = TwoColumnDocTemplate("test_frames4.pdf", pagesize=PAGESIZE)
        doc.build(story)
