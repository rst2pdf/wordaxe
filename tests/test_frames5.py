...
class TwoColumnDocTemplate(BaseDocTemplate):
    ...
    def __init__(self, filename, **kw):
        ...
        f1 = Frame(m, m+0.5*cm, cw+1.5*cm, ch-1*cm, id='F1', 
            leftPadding=0, topPadding=0, rightPadding=0, bottomPadding=0,
            showBoundary=True
        )
        f2 = Frame(cw+4*cm, m+0.5*cm, cw-2.0*cm, ch-1*cm, id='F2', 
            leftPadding=0, topPadding=0, rightPadding=0, bottomPadding=0,
            showBoundary=True
        )
...
class FrameSwitchTestCase(unittest.TestCase):
    ...
    with the second frame having a smaller width than the first one.
    """
    ...
    def test(self):    
        ...
        text = " ".join(['<a href="http://deco-cow.sourceforge.net">%s</a>' % w for w in text.split()])
        text += " "
        
        story = []
        story.append(Paragraph(text * 40, style=normal))

        doc = TwoColumnDocTemplate("test_frames5.pdf", pagesize=PAGESIZE)
        doc.build(story)
